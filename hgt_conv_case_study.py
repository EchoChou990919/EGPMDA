# Copied from ~/.local/lib/python3.8/site-packages/torch_geometric/nn/conv
...
class HGTConv(MessagePassing):
    ...
    def forward(
        ...
        for edge_type, edge_index in edge_index_dict.items():
            ...
            # Here I add "edge_type"
            out = self.propagate(edge_index, edge_type=edge_type, k=k, q=q_dict[dst_type], v=v,
    ...
    # Here I add "edge_type"
    def message(self, edge_type: str, k_j: Tensor, q_i: Tensor, v_j: Tensor, rel: Tensor,
        ...
        # Save the attention matrix of each meta-relation of each layer.  
        # Revealing how 1/2-hop messages are aggregated to the target miRNA / disease nodes, serving for explanation.  
        path = 'files/case_study/attentions/' + edge_type
        if not os.path.exists(path + '.npy'):
            np.save(path, alpha.cpu().numpy())
        else:
            np.save(path + '_1', alpha.cpu().numpy())
